...
// ─────────────────────────────────────────── Set to 1 to enable direct Wi-Fi
// telemetry posting from ESP32, or 0 for USB serial bridge mode
...
// ────────────────────────────────────────────── Sensor 0 (S0 / S1 Entry):
// Platform / Outside Side
#define TRIG1_PIN 4
#define ECHO1_PIN 14
...
// Direction Inversion (Set to 1 if sensors are mounted in opposite physical
// orientation)
...
// ──────────────────────── Doorway setup: normal doorway width is 60-150cm. Max
// echo timeout 10000us (~171cm).
#define PULSE_TIMEOUT_US                                                       \
  10000 // 10ms timeout (~171cm max range) avoids long blocking
#define THRESHOLD_ENTER_CM 75.0 // Blocked when distance < 75 cm
#define THRESHOLD_LEAVE_CM 85.0 // Cleared when distance > 85 cm

// Maximum time in milliseconds allowed for a full traversal before resetting to
// IDLE
#define TRAVERSAL_TIMEOUT_MS 1200

// Cooldown in milliseconds after a successful count before next crossing can
// start
#define COOLDOWN_MS 300

// Spacing delay in milliseconds between pinging S0 and S1 to avoid acoustic
// cross-talk
#define SENSOR_SPACING_MS 10

// Debug logging over Serial (0 = Clean JSON only for serial_bridge.py, 1 =
// Verbose debug)
